-- Aggregated insights for map filters (same semantics as filtered sightings).
-- Returns camelCase JSON keys for direct API responses.
...
with base as (
  ...
  where (p_species_id is null or a.species_id = p_species_id)
    and (
      ...
      or (
        s.longitude::double precision >= p_min_lng
        and s.longitude::double precision <= p_max_lng
        and s.latitude::double precision >= p_min_lat
        and s.latitude::double precision <= p_max_lat
      )
    )
